...
public interface PatientLiveStatusRepository extends JpaRepository<PatientLiveStatus, Integer> {
	...
	@Query(nativeQuery = true, value = "SELECT pls.patient_id patientID,pls.patient_condition patientCondition,pls.test_outcome testOutcome,f.name facilityName,w.name wardName,f.facility_id facilityId,w.id wardId FROM patient_live_status pls INNER JOIN facilities f ON f.facility_id=pls.facility_id LEFT JOIN wards w ON w.id=pls.ward_id WHERE pls.patient_id=?1")
